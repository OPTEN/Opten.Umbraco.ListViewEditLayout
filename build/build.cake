	...
	.Does(() =>
{
	...
	FileWriteText(dest + File("Opten.Umbraco.ListViewEditLayout.variables.txt"), "version=" + version);

	Information("Patch files with ?v: {0}", ReplaceTextInFiles(
       "../src/Opten.Umbraco.ListViewEditLayout.Web.UI/App_Plugins/**/*",
       "?v=ASSEMBLY_VERSION",
       "?v=" + version
	).Count());
...
Task("Restore-NuGet-Packages") 
	...
	.Does(() =>
{ 
	NuGetRestore("../Opten.Umbraco.ListViewEditLayout.sln", new NuGetRestoreSettings {
		NoCache = true
	}); 
});
...
Task("Build") 
	...
	.Does(() =>
{
	/*MSBuild("../src/Opten.Umbraco.ListViewEditLayout/Opten.Umbraco.ListViewEditLayout.csproj", settings =>
...
Task("Pack")
	...
	.Does(() =>
{
	NuGetPackWithDependencies("./Opten.Umbraco.ListViewEditLayout.nuspec", new NuGetPackSettings {
		Version = version,
		BasePath = "../",
...
Task("Deploy")
	.Does(() =>
{
	string packageId = "Opten.Umbraco.ListViewEditLayout";
	...
	Slack(new SlackSettings {
		ProjectName = "Opten.Umbraco.ListViewEditLayout"
	});
});
